...
public class MyAdjacencyListUndirectedGraph {
   ...
   * 创建图(自己输入数据)
   */
  public MyAdjacencyListUndirectedGraph() {
   ...
   *     edges -- 边
   */
  public MyAdjacencyListUndirectedGraph(char[] vexs, EData[] edges) {
  ...
  private void DFS(int i, boolean[] visited) {
    ...
    while (node != null) {
      if (!visited[node.ivex]) DFS(node.ivex, visited);
      node = node.nextEdge;
    }
  }
  ...
  private int getWeight(int start, int end) {
    ...
    while (node != null) {
      if (end == node.ivex) return node.weight;
      node = node.nextEdge;
    }
  ...
  public void prim(int start) {
    ...
    for (i = 0; i < num; i++) weights[i] = getWeight(start, i);
    ...
    for (i = 0; i < num; i++) {
      ...
      for (j = 0; j < num; j++) {
        ...
        if (weights[j] != 0 && tmp < weights[j]) weights[j] = tmp;
      }
    }
    ...
    for (i = 0; i < index; i++) System.out.printf("%c ", prims[i]);
    System.out.printf("\n");
  }
  ...
  public void kruskal() {
    ...
    int length = 0;
    for (int i = 0; i < index; i++) length += rets[i].weight;
    System.out.printf("Kruskal=%d: ", length);
    for (int i = 0; i < index; i++) System.out.printf("(%c,%c) ", rets[i].start, rets[i].end);
    System.out.printf("\n");
  }
  ...
  public static void main(String[] args) {
    ...
    MyAdjacencyListUndirectedGraph pG;

    // 自定义"图"(输入矩阵队列)
    // pG = new ListUDG();
    // 采用已有的"图"
    pG = new MyAdjacencyListUndirectedGraph(vexs, edges);
